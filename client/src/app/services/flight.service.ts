...
export class FlightService {
  ...
  getAllFlights(): Observable<Flights[]> {
    return this.http.get<Flights[]>(`${this.baseUrl}/api/flights`, { headers: this.getHeaders() });
  }
  ...
  searchFlights(source: string, destination: string, date: string): Observable<Flights[]> {
    ...
    return this.http.get<Flights[]>(`${this.baseUrl}/api/flights/search`, { headers: this.getHeaders(), params });
  }

  createFlight(flight: Flights): Observable<Flights> {
    return this.http.post<Flights>(`${this.baseUrl}/api/flights`, flight, { headers: this.getHeaders() });
  }

  updateFlight(id: number, flight: Flights): Observable<Flights> {
    return this.http.put<Flights>(`${this.baseUrl}/api/flights/${id}`, flight, { headers: this.getHeaders() });
  }
  ...
  getPilots(): Observable<User[]> {
    return this.http.get<User[]>(`${this.baseUrl}/api/pilot/schedule/users`, { headers: this.getHeaders() });
  }

  getAssignPilotDetails(): Observable<FlightSchedule[]> {
    return this.http.get<FlightSchedule[]>(`${this.baseUrl}/api/pilot/schedule/scheduleUser`, { headers: this.getHeaders() });
  }

  getAllAssignPilotDetails(): Observable<FlightSchedule[]> {
    return this.http.get<FlightSchedule[]>(`${this.baseUrl}/api/pilot/schedule`, { headers: this.getHeaders() });
  }

  updateFlightStatus(id: number, status: string): Observable<any> {
    return this.http.put(`${this.baseUrl}/api/pilot/schedule/${id}/status`, { status }, { headers: this.getHeaders() });
  }
  ...
  checkAvailability(flightId: number, travelerCount: number): Observable<any> {
    ...
    return this.http.get<any>(`${this.baseUrl}/api/flights/${flightId}/check-availability`, { headers: this.getHeaders(), params });
  }
}
